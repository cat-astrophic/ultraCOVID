# This script adds county level data from the USDA ERS to serve as proxies for individual atheletes
...
for i in range(len(ultradata)):
    ...
    fips.append(flipsadelphia(ultradata.iloc[i]))

# Adding FIPS codes to the main dataframe

fips = pd.Series(fips, name = 'FIPS')
ultradata = pd.concat([ultradata, fips], axis = 1)
...
ultradata = pd.concat([ultradata, edu_cc, edu_bs], axis = 1)
...
ultradata = pd.concat([ultradata, popu], axis = 1)
...
ultradata = pd.concat([ultradata, nojo, hhinc], axis = 1)
